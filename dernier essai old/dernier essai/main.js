gsap.registerPlugin(Draggable, gsap);
console.clear();


let dragEnd = 0;
let positionSong = 0;
let arm = document.querySelector("#arm");
let diskCenter = document.querySelector("#centerVinyle");
let isStarted = false;
let timelineTurn = gsap.timeline({ repeat: -1, defaults: { ease: "none" } });
let interval;
let player = document.querySelector("#player");
let songTimeEnd = player.duration;
let timeElapsed = player.currentTime;
var volumeControl = document.getElementById('vol-control');
var rate = document.querySelector("#rate");
let timelineTurnCenter = gsap.timeline({ repeat: -1, defaults: { ease: "none" } });
//let timelinePlay = gsap.timeline({ repeat: 0, defaults: { ease: "none" } });
let setRated = 1;
/////////////////////////////////
let testinter;
let rotateOnDragEnd;
//let ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * (gsap.getProperty("#arm", "rotation") - 76) / 17)) / (player.playbackRate)), onComplete: retourzerobras });
let contenus = {};
let ArmGO = TweenMax.to("#arm", { rotation: 70, duration: 2 });
//let ArmGO;
let rotatemin = gsap.getProperty("#arm", "rotation");
if (rotatemin == 0) {
    rotatemin = 77;
}

//let ArmGO = TweenMax.to("#arm", { rotation: 93, duration: 20, onComplete: retourzerobras });
//let ArmGO = TweenMax.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * ((gsap.getProperty("#arm", "rotation") - 76) / 17)))), onComplete: retourzerobras });
console.log("testdeplacement" + gsap.getProperty("#arm", "rotation") + " " + rotatemin);

var SetVolume = function() {
    player.volume = this.value / 100;
    console.log(this.value);
};

//let SetRate = function() {
rate.addEventListener('input', SetRate);

volumeControl.addEventListener('input', SetVolume);

timelineTurn.pause();
timelineTurnCenter.pause();
TweenLite.set('#arm', { transformOrigin: '54px 67px' });

//fait bouger le bras
const dragable = Draggable.create("#arm", {
    type: "rotation",
    transformOrigin: "50% 50%",
    throwProps: true,
    bounds: { minRotation: 70, maxRotation: 93 },


    //au deplacement
    onDragEnd: function() {
        play();
        // if (this.rotation > 76 && this.rotation < 93 && dragEnd == 0) {
        //     console.log("test");
        //     songTimeEnd = player.duration;
        //     timeElapsed = songTimeEnd - (songTimeEnd * (this.rotation - 76) / 17);
        //     player.currentTime = songTimeEnd - timeElapsed;
        //     let turn = Math.ceil(timeElapsed / 2);

        //     timelineTurn.play();
        //     timelineTurnCenter.play();
        //     player.play();
        //     rotateOnDragEnd = this.rotation;
        //     if (setRated == !1) {
        //         console.log(setRated + "ok");
        //         timelinePlay.clear();
        //         // timelinePlay
        //         //     .to("#arm", { rotation: 93, duration: (songTimeEnd - (songTimeEnd * (this.rotation - 76) / 17)) / setRated }, ">")
        //         //     .to("#arm", { rotation: 70, duration: 2 })
        //         //ArmGO.kill();
        //         ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * (rotateOnDragEnd - 76) / 17)) / SetRate), onComplete: retourzerobras });
        //         // let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2 })
        //     } else {
        //         console.log(setRated + "pas ok");
        //         timelinePlay.clear();
        //         timelinePlay.timeScale(1);
        //         // timelinePlay
        //         //     .to("#arm", { rotation: 93, duration: (songTimeEnd - (songTimeEnd * (this.rotation - 76) / 17)) / setRated }, ">")
        //         //     .to("#arm", { rotation: 70, duration: 2 })
        //         //ArmGO.kill();
        //         ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * (rotateOnDragEnd - 76) / 17))), onComplete: retourzerobras });
        //         //let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2 })
        //     }
        //     //diskTurn(turn);
        //     // timelineTurn.play();

        //     let pos = (this.rotation - 76) / 17;
        //     //interval = setInterval(testfinrotation, 500);
        //     testinter = setInterval(testaudio, 5000);
        //     console.log(timelinePlay.progress());
        //     console.log(this.rotation);
        //     dragEnd = 1;
        // }

    },
    onDragStart: function() {
        songTimeEnd = player.duration;
        dragEnd = 0;
        timelineTurn.pause();
        timelineTurnCenter.pause();
        player.pause();

    },
    onPress: function() {
        dragEnd = 0;
    },
    onRelease: function() {
        play();

    }
})

function play() {
    if (gsap.getProperty("#arm", "rotation") > 76 && gsap.getProperty("#arm", "rotation") < 93 && dragEnd == 0) {
        console.log("test");
        songTimeEnd = player.duration;
        timeElapsed = songTimeEnd - (songTimeEnd * (gsap.getProperty("#arm", "rotation") - 76) / 17);
        player.currentTime = songTimeEnd - timeElapsed;
        let turn = Math.ceil(timeElapsed / 2);

        timelineTurn.play();
        timelineTurnCenter.play();
        player.play();
        rotateOnDragEnd = gsap.getProperty("#arm", "rotation");
        // if (!ArmGO.isActive()) {
        //     ArmGO.kill();
        //     console.log("le tween est actif");
        // } else {
        //     ArmGO.seek(0).pause();
        // }

        //console.log(ArmGO.totalDuration() + "duration");
        //ArmGO.kill("duration");
        console.log(songTimeEnd);
        console.log("en minute" + new Date(songTimeEnd * 1000).toISOString().substr(11, 8));
        // ArmGO = null;
        // if (setRated == !1) {
        console.log(setRated + "ok");
        console.log(ArmGO.totalDuration() + "duration");

        ////////////////////test pour fixer la duration du tween
        //ArmGO.duration(((songTimeEnd - (songTimeEnd * (rotateOnDragEnd - 76) / 17))));


        //timelinePlay.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * ((rotateOnDragEnd - 76) / 17)))), onComplete: retourzerobras });
        ArmGO = TweenMax.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * ((rotateOnDragEnd - 76) / 17)))), onComplete: retourzerobras });
        ArmGO.resume();
        ArmGO.restart();
        PlayPauseButtonValue.checked = "true";
        //ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * (gsap.getProperty("#arm", "rotation") - 76) / 17)) / (player.playbackRate)), onComplete: retourzerobras });
        // let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2 })
        // } else {
        //     console.log(setRated + "pas ok");
        //timelinePlay.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * ((rotateOnDragEnd - 76) / 17)))), onComplete: retourzerobras });

        //ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * ((rotateOnDragEnd - 76) / 17)))), onComplete: retourzerobras });
        //let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2 })
        // }


        let pos = (gsap.getProperty("#arm", "rotation") - 76) / 17;
        //interval = setInterval(testfinrotation, 500);
        testinter = setInterval(testaudio, 2000);
        //console.log(timelinePlay.progress());
        console.log(gsap.getProperty("#arm", "rotation"));
        dragEnd = 1;
    }

}

function SetRate() {
    let vitesse = (this.value / 100);
    player.playbackRate = vitesse.toFixed(2);
    timelineTurn.timeScale(player.playbackRate.toFixed(2));
    timelineTurnCenter.timeScale(player.playbackRate.toFixed(2));
    setRated = player.playbackRate.toFixed(2);
    play();
    vachier();
    // dernier qui marche
    ArmGO.resume;
    ArmGO.timeScale(player.playbackRate.toFixed(2));


    //timelinePlay.clear();
    //timelinePlay
    //ArmGO.resume;
    //ArmGO = gsap.to("#arm", { rotation: 93, duration: ((songTimeEnd - (songTimeEnd * (gsap.getProperty("#arm", "rotation") - 76) / 17)) / (player.playbackRate)), onComplete: retourzerobras });
    //let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2 })


};

function vachier() {
    console.log("vachier")
}




function testaudio() {
    console.log("duree :" + gsap.getProperty("#arm", "rotation"));
    let rotate = arm.style.transform;
    rotate = rotate.slice(rotate.indexOf("rotate(") + 7, rotate.indexOf("."))
    if (rotate <= 74) {
        clearInterval(testaudio);
    }
}
// function testaudio() {
//     console.log("duree :" + player.duration);
//     let rotate = arm.style.transform;
//     rotate = rotate.slice(rotate.indexOf("rotate(") + 7, rotate.indexOf("."))
//     if (rotate <= 74) {
//         clearInterval(testaudio);
//     }
// }

function testfinrotation() {
    let rotate = arm.style.transform;
    rotate = rotate.slice(rotate.indexOf("rotate(") + 7, rotate.indexOf("."))
    console.log(rotate);


    if (rotate <= 76) {
        timelineTurn.pause();
        timelineTurnCenter.pause();
        clearInterval(interval);
        stopAudio();
    }

}

/////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////Fonction qui arrete le disque de tourner

function StopDiskRotate() {
    timelineTurn.pause();
    timelineTurnCenter.pause();
}


/////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////// Retour à zero du bras puis arret rotation
function retourzerobras() {
    dragEnd = 0;
    let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2, onComplete: StopDiskRotate })
}

/////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////// changement de disque
function diskChange(index) {
    console.log("log de disk change" + index);
    dragEnd = 0;
    let ArmEnd = gsap.to("#arm", { rotation: 70, duration: 2, onComplete: playButton(index) });
    timelineTurn.pause();
    timelineTurnCenter.pause();
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////// Fonction PAuse Audio
function stopAudio() {
    player.pause();
}

///////////////////////////////////////////////////////////////////////////////////////////////////
timelineTurn
    .to("#disk", {
        duration: 2,
        rotation: 720,
        repeat: -1,
        ease: "none",
        paused: false
    });


timelineTurnCenter.to("#centerVinyle", {
    duration: 2,
    rotation: 720,
    repeat: -1,
    ease: "none",
    paused: false
})

///////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////          Fetch de la liste de musique

var myInit = {
    method: 'GET',
};

var p = fetch('music.json', myInit)
p.then(async function(response) {
        if (!response.ok) {
            throw new Error("HTTP error, status = " + response.status);
        }
        var contentType = response.headers.get("content-type");
        if (contentType && contentType.indexOf("application/json") !== -1) {
            return response.json()
                .then(function(contenu) {
                    //console.log(contenu);
                    contenus = contenu;
                    //player.src = contenu[1].music_src;
                    imageOnTop();
                })
        } else {

            console.log("le fichier envoyé n'est pas du json !");
        }


    })
    .catch(function(error) {
        console.log("le srv est inaccessible:");
        // Network Error!
        console.log(error);
    });

///////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////             pour afficher les disques en haut
let i = 1;
let y = 0;


function imageOnTop() {
    let zz = 0;
    //document.querySelector(".diskCenter").src = contenus[2].image;
    document.querySelectorAll(".diskCenter").forEach(element => {
        // document.querySelector('#diskvinylecenter' + i).src = contenus[y].image;
        element.src = contenus[zz].image;
        console.log("test avant le clic" + zz);
        element.setAttribute("data-index", zz);
        element.addEventListener("click", () => {
            console.log("test de clic" + zz);
            let index = element.getAttribute("data-index");
            diskChange(index);
            stopAll();
            //console.log(index);
            player.src = contenus[index].music_src;
            console.log("dans clic" + index);
            console.log(player.src);
        })
        console.log("sans clic" + zz);
        zz++;
    });
}

//////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////                   Fonction stop qui arrete tout
function stopAll() {
    // if (!ArmGO.isActive()) {
    //     ArmGO.play();
    //     console.log("le tween est actif");
    // } else {
    //     ArmGO.seek(0).pause();
    // }
    //timelinePlay.kill("#arm");
    //timelinePlay.pause;
    //timelinePlay.resume;
    // ArmGO.kill("duration");
    // ArmGO.kill();
    ArmGO.pause();
    // ArmGO = null;
    // ArmGO = TweenMax.to("#arm", { rotation: 93, duration: 0 });
    player.pause();
    player.currentTime = 0;
    //ArmGO.progress(0);
    // var Tweens = TweenMax.getTweensOf(ArmGO);
    // for (var i; i < Tweens.length; i--) {
    //     Tweens[i].repeat(0).progress(1).kill();
    // }
    //retourzerobras();

    //ArmGO = gsap.to("#arm", { rotation: 70, duration: 100 });

}


/////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////fonction bouton Play
let PlayPauseButtonValue = document.querySelector("#playPauseButton");
PlayPauseButtonValue.addEventListener("click", function() {
    console.log(document.querySelector("#playPauseButton").checked);


    if (this.checked == true) {
        //        playButton();
    } else {
        retourzerobras();
        stopAll();
    }
})

function playButton(index) {
    let ArmPlay = TweenMax.to("#arm", { rotation: 76.01, duration: 3, onComplete: play });
    diskCenter.src = contenus[index].image;
}

////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////// tranlation disk

function tranlateUpDisk() {
    let disk1TransUp = gsap
}


////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////    pour rendre cliquable les disque en haut, arret de la lecture et attribuer la source


// let z = 0;
// document.querySelectorAll(".Entiredisk").forEach(function element(item, index) {
//     item.addEventListener("click", lecteur(index));
//     // z++;
//     // if (z == 3) {
//     //     z = 0;
//     // }
//     console.log(index);
// });

// function lecteur1() {
//     diskChange(0);
//     stopAll();

//     //console.log(index);
//     player.src = contenus[0].music_src;
// }

// function lecteur2() {
//     diskChange(1);
//     stopAll();

//     //console.log(index);
//     player.src = contenus[1].music_src;
// }

// function lecteur3() {
//     diskChange(2);
//     stopAll();

//     //console.log(index);
//     player.src = contenus[2].music_src;
// }

// function lecteur4() {
//     diskChange(3);
//     stopAll();

//console.log(index);
// player.src = contenus[3].music_src;
// }

// disk1.addEventListener("click", lecteur1);
// disk2.addEventListener("click", lecteur2);
// disk3.addEventListener("click", lecteur3);
// disk4.addEventListener("click", lecteur4);


//le disque qui tourne
//function diskTurn(value) {


// let tween = gsap.to("#disk", {
//     duration: 2,
//     // x: 750, 
//     rotation: 720,
//     repeat: value,
//     ease: "none",
//     paused: false
// });


// console.log(value);
// return tween;

//}

// function diskNoTurn() {
//     let tween = gsap.to("#disk", {
//         duration: 2,
//         // x: 750, 
//         rotation: 360,
//         repeat: -1,
//         ease: "none",
//         paused: true
//     });
//     return tween;
// }






/*
 ** SCROLL
 */


// var scrollDistance = document.body.clientHeight / 2;
// var winScrollY;
// const timelineScrolled = gsap.timeline({
//     paused: true,
//     defaults: {
//         ease: 'power2.inOut',
//         duration: 1,
//     },
// });

// timelineScrolled
// .from('.main__content', { opacity: 0, y: 300 })
// .to(".sun", { rotation: 27, x: 100 }, "-=1.5")

// function scrolled() {
//     winScrollY = window.scrollY;
//     timelineScrolled.progress(winScrollY / scrollDistance);
// }
// scrolled();

// window.addEventListener("scroll", scrolled);